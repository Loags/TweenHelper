...
For visual review, open `Validation/Scenes/TweenHelperPresetReview.unity` and enter Play Mode. The review now contains 336 entries: 300 presets, 13 UI recipes, 13 validated collection entries, and 10 destination-motion entries covering every world/local pair. Existing status keys are unchanged, so all 326 completed results remain preserved while only the destination entries appear under **NOT REVIEWED**. Marking an entry still advances and automatically plays the next item.

...
namespace LB.TweenHelper
{
    ...
    internal static class BreatheFactory
    {
        public static Tween Create(GameObject target, float duration, TweenOptions options, float scaleAmount, Ease defaultEase)
        {
            ...
            return DOTween.Sequence()
                .Append(t.DOScale(originalScale * (1f + scaleAmount * strength), halfDur).SetEase(expandEase))
                .Append(t.DOScale(originalScale, halfDur).SetEase(contractEase))
    ...
    /// <summary>
    /// Continuously pulses scale between original and 1.08x in a smooth breathing rhythm using a repeating sequence.
    /// <para>
    /// Alternates between expanding to <c>originalScale * 1.08</c> and contracting back to original scale,
    /// each leg taking half the total duration. The returned sequence owns the complete loop,
    /// so delay is applied only on the first cycle and killing it stops all motion.
    /// </para>
    /// <para>
    /// <b>Type:</b> Looping (sequence) | <b>Default duration:</b> 4.0s (2.0s per leg) | <b>Default ease:</b> InOutSine<br/>
    ...
    /// Uses one repeating sequence; delay applies only on the first cycle.
    /// </para>
    /// <para>
    /// <b>Type:</b> Looping (sequence) | <b>Default duration:</b> 0.8s per cycle | <b>Default ease:</b> OutQuad (beat), InQuad (return)<br/>
    ...
    /// </para>
    /// <para>
    /// <b>Type:</b> Looping (sequence) | <b>Default duration:</b> 6.0s (3.0s per leg) | <b>Default ease:</b> InOutCubic<br/>

...
namespace LB.TweenHelper
{
    ...
    {
        public static TweenHandle Build(TweenRecipe recipe, IReadOnlyList<TweenPlayerBinding> bindings, GameObject owner, bool useUnscaledTime = false, UpdateType updateType = UpdateType.Normal, TweenMotionPreference motionPreference = TweenMotionPreference.UseProjectDefault)
        {
            if (!TryBuild(recipe, bindings, owner, out TweenHandle handle, out TweenRecipeValidationResult validation, useUnscaledTime, updateType, motionPreference))
            {
        ...
        public static bool TryBuild(TweenRecipe recipe, IReadOnlyList<TweenPlayerBinding> bindings, GameObject owner, out TweenHandle handle, out TweenRecipeValidationResult validation, bool useUnscaledTime = false, UpdateType updateType = UpdateType.Normal, TweenMotionPreference motionPreference = TweenMotionPreference.UseProjectDefault)
        {
            handle = null;
            ...
            try
            {
                ...
                for (int i = 0; i < recipe.Nodes.Count; i++)
                {
                    ...
                    GameObject secondaryTarget = TweenRecipeOperationCatalog.UsesSecondaryBinding(node.Operation)
                        ...
                        : null;
                    Tween tween = TweenRecipeOperationCatalog.CreateTween(node, target, secondaryTarget, playerBinding.Targets, TweenMotion.Resolve(motionPreference));

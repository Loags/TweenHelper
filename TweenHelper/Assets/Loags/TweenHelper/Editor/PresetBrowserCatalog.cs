...
namespace LB.TweenHelper.Editor
{
    internal static class PresetBrowserCatalog
    {
        ...
        private static IEnumerable<PresetBrowserEntry> CreateCollectionEntries()
        {
            ...
            yield return Collection("GridSerpentine", "Reveals grid items through alternating rows or columns from a selected corner.", "0.32s per item", "items.GridSerpentine(owner, columns: 3);", PresetBrowserPreviewKind.Grid, PresetBrowserCollectionKind.GridSerpentine, "Rows from top-left");
            ...
            yield return Collection("CollectionDealIn", "Deals items from one shared origin into their authored positions.", "0.45s per item", "items.CollectionDealIn(owner, origin);", PresetBrowserPreviewKind.Grid, PresetBrowserCollectionKind.CollectionDealIn, "First to last");
            yield return Collection("CollectionDealOut", "Deals items from authored positions into one shared destination.", "0.4s per item", "items.CollectionDealOut(owner, destination);", PresetBrowserPreviewKind.Grid, PresetBrowserCollectionKind.CollectionDealOut, "Last to first");
            yield return Collection("CollectionLayoutTransition", "Animates active direct UI children between two valid Unity-authored layouts.", "0.35s", "CollectionLayoutSnapshot before = container.CaptureCollectionLayout();\nitems.Reverse();\nApplySiblingOrder(items);\ncontainer.TweenCollectionLayoutFrom(before, 0.35f);", PresetBrowserPreviewKind.CollectionLayout, PresetBrowserCollectionKind.CollectionLayoutTransition, "Vertical list reorder");
        ...
        private static IEnumerable<PresetBrowserEntry> CreateTextEntries()
        {
            ...
            yield return Operation(PresetBrowserOperation.TypewriterReveal, "Reveals text by character, word, or line.", category, family, "0.85s", "label.TypewriterReveal(TextAnimationUnit.Word);", badge, preview);
            yield return Operation(PresetBrowserOperation.TypewriterHide, "Hides text by character, word, or line.", category, family, "0.65s", "label.TypewriterHide(TextAnimationUnit.Word);", badge, preview);
            ...
            yield return Operation(PresetBrowserOperation.TextStaggerIn, "Staggers characters, words, or lines into place.", category, family, "0.65s", "label.TextStaggerIn(TextAnimationUnit.Word, StaggerOrder.FromCenter, unitStagger: 0.08f);", badge, preview, "Up");
            yield return Operation(PresetBrowserOperation.TextStaggerOut, "Staggers characters, words, or lines out of view.", category, family, "0.58s", "label.TextStaggerOut(TextAnimationUnit.Line, StaggerOrder.LastToFirst, unitStagger: 0.35f);", badge, preview, "Up");
            ...
            yield return Operation(PresetBrowserOperation.TextWiggle, "Plays one smooth deterministic position and rotation cycle.", category, family, "0.65s", "label.TextWiggle(seed: 1337);", badge, preview);
            yield return Operation(PresetBrowserOperation.TextFloat, "Plays one phase-offset directional float cycle.", category, family, "0.9s", "label.TextFloat(UISequenceDirection.Up);", badge, preview, "Up");
            yield return Operation(PresetBrowserOperation.TextSwing, "Swings glyphs around a center or top pivot.", category, family, "0.8s", "label.TextSwing(pivot: TextGlyphPivot.Top);", badge, preview);
            yield return Operation(PresetBrowserOperation.TextPulse, "Pulses per-glyph scale for one finite cycle.", category, family, "0.7s", "label.TextPulse();", badge, preview);
            yield return Operation(PresetBrowserOperation.TextScatterIn, "Resolves deterministic scattered poses into authored text.", category, family, "0.75s", "label.TextScatterIn(order: StaggerOrder.Random, seed: 1337);", badge, preview);
            yield return Operation(PresetBrowserOperation.TextScatterOut, "Scatters authored text into deterministic poses and finishes hidden.", category, family, "0.65s", "label.TextScatterOut(order: StaggerOrder.Random, seed: 1337);", badge, preview);
            yield return Operation(PresetBrowserOperation.TextRotateIn, "Rotates ordered glyph groups into authored text.", category, family, "0.65s", "label.TextRotateIn(order: StaggerOrder.FromCenter);", badge, preview);
            yield return Operation(PresetBrowserOperation.TextRotateOut, "Rotates ordered glyph groups out and finishes hidden.", category, family, "0.58s", "label.TextRotateOut(order: StaggerOrder.ToCenter);", badge, preview);
            yield return Operation(PresetBrowserOperation.TextShear, "Applies a finite horizontal glyph deformation.", category, family, "0.6s", "label.TextShear();", badge, preview);
            yield return Operation(PresetBrowserOperation.TextTrackingPulse, "Expands glyphs from the visual center without changing layout.", category, family, "0.7s", "label.TextTrackingPulse();", badge, preview);
            yield return Operation(PresetBrowserOperation.TextImpactRipple, "Propagates a finite reaction from a TMP-local impact point.", category, family, "0.75s", "label.TextImpactRipple(Vector2.zero);", badge, preview);
